...
var statFunctions = {
  ...
  setPropMDA: function(regimen) {
    var ps = simulationEngine.modelParams();
...
var simulationEngine = {
  ...
  scenarioRunStats: () => {
    ...
    var stats = simulationEngine.reductionStatsCalc(scenario, params.covMDA);
    ...
    simulationEngine.dump(scenario);
  ...
  reductionStatsCalc: (scenario, coverage) => {
    ...
    for (var t = 0; t < T; t++) {
      ...
      var mM = [],
        ...
        mL = [];
      for (var i = 0; i < n; i++) {
        var prev0 = (prev = scenario["results"][i]["Ms"][0]);
        var red = scenario["results"][i]["Ms"][t] / prev0;
        var prev = scenario["results"][i]["Ms"][t];
      ...
      doses[t] = doses[t] / n;
      medM[t] = simulationEngine.median(mM);
      medW[t] = simulationEngine.median(mW);
      medL[t] = simulationEngine.median(mL);
    }
  ...
  modalConfirmation: function() {
    ...
    console.log(this);
    simulationEngine.fixInput(false);

    if (SessionData.ran(i)) {
      ScenarioIndex.setIndex(i);
      $("#settings-modal").modal("hide");
    } else {
      simulationEngine.runSimClick();
    }
  },
  runSimClick: function() {
  ...
  documentReady: function() {
    ...
    simulationEngine.modalConfirmation();
    ...
    /*     $(document).ready(function() {
      ...
      $("#map-progress-bar").hide();
      $("#run_scenario").on("click", simulationEngine.modalConfirmation);
  ...
  toggle: (className, obj) => {
    var $input = $(obj);
    if ($input.prop("checked")) $(className).show();
    else $(className).hide();
  },

  toggleOff: (className, obj) => {
    var $input = $(obj);
...
// simulationEngine.documentReady();
