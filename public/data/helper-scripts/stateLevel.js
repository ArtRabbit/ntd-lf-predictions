          ...
          const myAsync = async () => {
            await asyncForEach(result, element => {
              ...
              const exists2 = countryArrayofObjects[countryCode].find(
                params => {
                  return params == element.StateCode
                }
              )
              if (!exists2) {
                countryArrayofObjects[countryCode].push(element.StateCode)
                console.log(element.StateCode)
                //                  const myPromise = await con.query("SELECT * FROM StateLevel WHERE StateCode ='" + element.state_code + "'") // not working
                con.query(
                  "SELECT * FROM StateLevel WHERE StateCode ='" +
                    element.StateCode +
                    "'",
