...
con.connect(function(err) {
  ...
  con.query('SELECT * FROM CountryLevel', function(err, result, fields) {
    if (err) throw err

    result.forEach(element => {
      noSpaceCountry = element.Country.replace(/ /g, '')
      ...
      if (!exists) {
        ...
        countryArrayofObjects[noSpaceCountry] = []
      }
    })
    console.log(countryArray)
    console.log(countryArrayofObjects)
    // console.log(countryArray.length, 'countries')

    /***** gather regimes for each country *****/
    //    let regimes = Object.keys(countryArray)
    countryArray.forEach(country => {
      //   console.log(country)
      let thisCountry = []
      con.query(
        "SELECT * FROM CountryLevel WHERE Country ='" + country + "'",
        function(err, result, fields) {
          ...
          result.forEach((element2, stateIndex) => {
            thisCountry[stateIndex + 1] = []
            let valuesOnly = Object.values(element2)
            //console.log(valuesOnly)
            valuesOnly.forEach((element3, valueIndex) => {
              if (valueIndex !== 0) {
                //   console.log(element3)
                thisCountry[stateIndex + 1].push(element3)
              }
            })
            console.log(thisCountry)

            let outputString = thisCountry.map(e => e.join(',')).join('\r\n')
            let noSpaceCountry = country.replace(/ /g, '')
            fs.writeFileSync(
              './country-level/' + noSpaceCountry + '.csv',
              outputString,
              'utf-8'
            )
          })
